...
def commingSoon():
    print("[deep_pink3]Comming Soon[/deep_pink3]")

def createMenu():
    menubar = tkinter.Menu(window)
    file_menu = tkinter.Menu(window, tearoff=0)
    file_menu.add_command(label="Small (10x10 with 10 mines)", command=lambda: setSize(10, 10, 10))
    file_menu.add_command(label="Medium (20x20 with 40 mines)", command=lambda: setSize(20, 20, 40))
    file_menu.add_command(label="Big (35x35 with 120 mines)", command=lambda: setSize(35, 35, 120))
    file_menu.add_command(label="Custom", command=setCustomSize)
    file_menu.add_separator()
    file_menu.add_command(label="Restart", command=restartGame)
    file_menu.add_command(label="Exit", command=window.destroy)

    view_menu = tkinter.Menu(window, tearoff=0)
    view_menu.add_command(label="Comming Soon", command=commingSoon)
    for x in range(0, len(customsizes)):
        file_menu.add_command(label=str(customsizes[x][0])+"x"+str(customsizes[x][1])+" with "+str(customsizes[x][2])+" mines", command=lambda customsizes=customsizes: setSize(customsizes[x][0], customsizes[x][1], customsizes[x][2]))
...
def saveConfig():
    ...
    with open("mines_config.ini", "w") as file:
        config.write(file)

def loadConfig():
    global rows, cols, mines, customsizes
    config = configparser.ConfigParser()
    config.read("mines_config.ini")
